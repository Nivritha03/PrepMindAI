...
const floatingOrbs = [
  { size: 600, top: '-20%', left: '-10%', color: 'from-orange-600/20 to-red-600/20', delay: 0 },
  { size: 400, top: '40%', right: '-5%', color: 'from-rose-600/20 to-pink-600/20', delay: 2 },
  { size: 300, bottom: '-10%', left: '30%', color: 'from-yellow-600/20 to-orange-500/20', delay: 4 },
];
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0505] text-white overflow-x-hidden">
      ...
      <nav className="relative z-50 flex justify-between items-center px-8 md:px-16 py-6">
        <MagneticButton>
          <div className="flex items-center gap-3 cursor-hover">
            <div className="p-2 bg-gradient-to-br from-orange-500 to-rose-500 rounded-xl shadow-[0_0_20px_rgba(244,63,94,0.5)]">
      ...
      <motion.section style={{ y: heroY, opacity: heroOpacity }} className="relative z-10 text-center px-6 pt-20 pb-32 max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-card border border-rose-500/30 text-rose-300 text-sm font-medium mb-8 cursor-hover">
        ...
        <h1 className="text-6xl md:text-8xl font-black tracking-tight leading-none mb-8">
          {titleWords.map((word, i) => (
            <motion.span
              ...
              className={`inline-block mr-4 ${i >= 2 ? 'bg-gradient-to-r from-orange-400 via-rose-400 to-yellow-400 bg-clip-text text-transparent block mt-2' : ''}`}
            >
              {word}
            </motion.span>
          ))}
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.7 }}
          ...
          <MagneticButton onClick={() => navigate('/signup')} className="group flex items-center gap-3 px-10 py-5 bg-gradient-to-r from-orange-500 to-rose-500 text-white font-bold rounded-2xl hover:scale-105 transition-all shadow-[0_0_40px_rgba(244,63,94,0.4)] text-lg cursor-hover">
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8">
          {stats.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center cursor-hover">
              <div className="text-4xl md:text-5xl font-black bg-gradient-to-r from-orange-400 to-rose-400 bg-clip-text text-transparent mb-2">{s.number}</div>
      ...
      <section className="relative z-10 py-32 px-6 max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-20 cursor-hover">
          <h2 className="text-5xl md:text-6xl font-black tracking-tight mb-6">
            ...
            <span className="block bg-gradient-to-r from-orange-400 to-rose-400 bg-clip-text text-transparent">land your dream job</span>
          </h2>
      ...
      <section className="relative z-10 py-32 px-6">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-orange-500/10 via-rose-500/10 to-transparent rounded-[3rem]" />
          ...
          <Shield size={48} className="text-orange-400 mx-auto mb-6 relative z-10" />
